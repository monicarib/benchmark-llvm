...
static long fann(int n , long *vet)
{
    Aint*	perm;
    Aint*	perm1;
    Aint*	count;
	...
	if( didpr < 30 ) {
	    ...
	    for ( i=0 ; i<n ; ++i ){
                ...
                if      (i == 0) valor = (int)(1+perm1[i]);
                else if (i < 9)  valor = valor * 10 + (int)(1+perm1[i]);
                else             valor = valor * 100 + (int)(1+perm1[i]);
            }
	    vet[didpr] = valor; //printf("\n");
	    ++didpr;
	}
	for( ; r!=1 ; --r ) {
	    count[r-1] = r;
...
//long fannkuch( int argc, char* argv[] )
long fannkuch(long *vet)
{
    int		n = 11;
